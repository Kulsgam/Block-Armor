...
public class SetEffectSlow_Motion extends SetEffect 
{
	protected SetEffectSlow_Motion() {
		...
		this.potionEffects.add(new MobEffectInstance(MobEffects.SLOWNESS, 10, 2, true, false));
	}
	...
	public void onArmorTick(Level world, Player player, ItemStack stack) {
		...
		// particles
		if (world.isClientSide() && world.getRandom().nextInt(10) == 0 && 
				(player.getDeltaMovement().x != 0 || player.getDeltaMovement().z != 0 || !player.onGround())) 
			world.addParticle(ParticleTypes.SOUL, player.getX()+world.getRandom().nextDouble()-0.5D, 
					player.getY()+world.getRandom().nextDouble()+0.3d, player.getZ()+world.getRandom().nextDouble()-0.5D, 
					0, 0, 0);
