		...
		if (BlockArmor.key.isKeyDown(player) && ArmorSet.getFirstSetItem(player, this) == stack) {
			...
			if (!world.isClientSide()) {
				((ServerLevel)world).sendParticles(ParticleTypes.WHITE_ASH, player.getX()+(world.getRandom().nextDouble()-0.5D)*radius, 
						player.getY()+world.getRandom().nextDouble()+2D, player.getZ()+(world.getRandom().nextDouble()-0.5D)*radius, 
						5, 0, 0, 0, 0);
				for (int i=0; i<2; ++i)
					((ServerLevel)world).sendParticles(ParticleTypes.CLOUD, player.getX()+(world.getRandom().nextDouble()-0.5D)*radius, 
							player.getY()+world.getRandom().nextDouble()*0.5d+2.5D, player.getZ()+(world.getRandom().nextDouble()-0.5D)*radius, 
							5, 0, 0, 0, 0);
				if (world.getRandom().nextInt(2) == 0)
					world.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.WEATHER_RAIN, 
							player.getSoundSource(), 0.1f, world.getRandom().nextFloat());
				for (int x=-radius/2; x<=radius/2; x++)
					for (int z=-radius/2; z<=radius/2; z++)
						for (int y=0; y<=2; y++) {
							BlockPos pos = player.blockPosition().offset(x, y, z);
							BlockPos posBelow = player.blockPosition().offset(x, y-1, z);
							if (player.mayBuild() && world.getRandom().nextInt(100) == 0 && world.isEmptyBlock(pos)) {
				...
				if (world.getRandom().nextInt(15) == 0) {
					ItemEntity item = new ItemEntity(world, player.getX()+(world.getRandom().nextDouble()-0.5D)*radius, 
							player.getY()+world.getRandom().nextDouble()+1.5D, player.getZ()+(world.getRandom().nextDouble()-0.5D)*radius,
							new ItemStack(Items.SNOWBALL));
					item.setPickUpDelay(40);
					world.addFreshEntity(item);
					world.playSound(player, player.getX(), player.getY(), player.getZ(), SoundEvents.SNOW_STEP, 
							player.getSoundSource(), 1.0f, world.getRandom().nextFloat());
				}
			}
		}
	}
